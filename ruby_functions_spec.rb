...
class Functions_Practice < MiniTest::Test
  ...
  def test_volume_of_sphere()
    volume_result= sphere(1)
    x= 4/3 * Math::PI * 1**3
    assert_equal( x, volume_result)
  end
  ...
  #I have managed to write a code that works but I cant figure out how to test it in this spec.rb file. I dont have anything to pass to the method so I can't figure out what the test would be. I know it works because I changed "return" to "puts" and I ran the code in the terminal under a different file name. The code here is the same as I have on my "ruby_functions_practice.rb" file, so when I run this file I get "1 runs, 0 assertions, 0 failures, 0 errors, 0 skips".



  #Given a date of birth, calculate how old a person born on that date would be
  def test_age_of_person()
    age = how_old(1986, 9, 25)
    assert_equal(29, age)
  end
